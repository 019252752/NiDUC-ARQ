...
def odbieranie_sygnalu(sygnal):
    ...
    #sprawdzamy, czy ramka ktora odebral receiver jest ta, ktorej oczekiwal - jezeli nie, to ignorujemy ja
    ...
    if(parzystosc % 2 != sygnal[len(sygnal)-1]):
        append_NAK(ACKK)
        #sygnal zostal zaklocony
    else:
        #sygnal nie zostal zaklocony
        append_ACK(ACKK)
    "".join(str(ACKK))
...
def zaklocenia(sygnal):
    for i in range (0, len(sygnal)): #wszystko moze byc zaklocone, czy to numer sekwencji, czy to ramka wlasciwa, czy to suma kontrolna, czy nawet ACK/NAK
        if(random.randint(0,9) == 9): #tutaj zmieniamy szanse na zaklocenie
            if(sygnal[i] == 1): sygnal[i] = 0
            else: sygnal[i] = 1

dlugosc_bit = 512 #ile bitow powinna zawierac nasza wiadomosc
...
while (x != dlugosc_bit // 8): #dopoki nie wyslalismy wszystkich bitow
    ...
    for a in range(0, 4):  # pojedyncze okno
        if(dlugosc_bit > (x + a)*8): #na wypadek, jakby skonczyly sie bity do przeslania przed koncem okna
            ...
            wysylanie_sygnalu_CRC(ramka) #przesylamy ramke z dana suma kontrolna
            ramki_przeslane = ramki_przeslane + 1

            if(ACK == 1): numer_ramki = numer_ramki + 1 #jezeli receiver przyjal wlasciwa ramke, to bedzie oczekiwal ramki o kolejnym numerze sekwencji
