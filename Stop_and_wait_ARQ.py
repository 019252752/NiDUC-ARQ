...
def odbieranie_sygnalu_CRC(sygnal):
    ...
    if(sprawdzanie_CRC(sygnal) == 1): #jezeli wynik dzielenia byl rowny 1, czyli jezeli ramka zostala zaklocona
        append_NAK(ACKK)
    else:
...
dlugosc_bit = 512 #ile bitow powinna zawierac nasza wiadomosc, zmieniamy to w trakcie testow
informacja = list("") #jakis ciag bitow
informacja = generowanie_sygnalu(informacja, dlugosc_bit) #generujemy losowy ciag bitow
"".join(str(informacja))
print(informacja)
czas1 = time.time() #rozpoczecie odmierzania czasu
for i in range (0, dlugosc_bit // 8): #wczytujemy pojedyncza ramke
    ...
    while(ACK == 0): #dopoki receiver nie otrzyma wlasciwej ramki
        ...
        oryginalna_ramka = list(ramka)  # do sprawdzania, czy zaakceptowana zostala odpowiednia ramka (receiver moze zaakceptowac bledna ramke)
        print("Ramka numer " + str(i+1))
        wysylanie_sygnalu_CRC(ramka) #wysylamy kolejna ramke, zmieniaj tu sposob wysylania
...
print("Ilość ramek poprawnie odebranych: ", ramki_poprawne) #ile ramek zostalo poprawnie odebranych (tzn. ramka zostala zaakceptowana I nie byla zaklocona)
